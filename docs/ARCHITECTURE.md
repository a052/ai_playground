...
- **URL safety**: `src/lib/urlSafety.ts` (`safeHttpUrl`) allows only `http:` / `https:`. `runWebSearch` blanks non-http(s) result URLs after parse; `urlFetcher.normalizeUrl` rejects non-http(s) schemes (and only prepends `https://` for scheme-less hosts); `ToolCallCard` only renders an `<a href>` when the hit is http(s), otherwise plain text — so a compromised/custom search provider cannot XSS via `javascript:` links (keys live in localStorage).
